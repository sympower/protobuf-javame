...
import net.jarlehansen.protobuf.javame.examples.generated.Address;
...
public class ProtobufJavaMeMidlet extends MIDlet implements CommandListener {
	...
	protected void startApp() {
		...
		try {
			logger.info("Creating example objects");
			final Address address = Address.newBuilder().setStreet("Highway 1").build();
			final ExampleObject exampleObject = ExampleObject.newBuilder().setId(1).setName(
					"John").setAddress(address).build();
