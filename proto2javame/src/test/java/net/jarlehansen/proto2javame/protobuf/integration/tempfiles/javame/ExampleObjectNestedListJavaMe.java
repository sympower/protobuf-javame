	...
	static boolean populateBuilderWithField(final InputReader reader, final Builder builder, final int fieldNumber) throws IOException {
		...
		switch (fieldNumber) {
			...
			case fieldNumberPersons:
				Vector vcPersons = reader.readMessages(fieldNumberPersons);
				for(int i = 0 ; i < vcPersons.size(); i++) {
					byte[] eachBinData = (byte[]) vcPersons.elementAt(i);
					Person.Builder builderPersons = Person.newBuilder();
					InputReader innerInputReader = new InputReader(eachBinData, unknownTagHandler);
					boolean boolPersons = true;
					int nestedFieldPersons = -1;
					while(boolPersons) {
						nestedFieldPersons = getNextFieldNumber(innerInputReader);
						boolPersons = Person.populateBuilderWithField(innerInputReader, builderPersons, nestedFieldPersons);
					}
					eachBinData = null;
